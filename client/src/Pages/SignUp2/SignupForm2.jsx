  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const userResponse = await axios.post("http://localhost:3001/user", {
        ...
        tel: formData.get("phonenumber"),
      });

      if (userResponse.data) {
        // Create a new Manager or Owner based on the selected user type
        let managerOrOwnerResponse;
        if (userType === "manager") {
          managerOrOwnerResponse = await axios.post(
          ...
          managerOrOwnerResponse = await axios.post(
            "http://localhost:3001/owner",
            {
              ...
              lastname: formData.get("lastName"),
              email: formData.get("email"),
              backuptel: formData.get("backuptel"),
              userid: userResponse.data.userid,
            }
          );
        }

        if (managerOrOwnerResponse.data) {
          navigate("/dashboard"); // Change "/dashboard" to the desired route after successful signup
        } else {
          throw new Error("Failed to create manager or owner");
        }
      } else {
          ...
          <Grid item xs={12} sm={8} elevation={6} p={4}>
            ...
            >
              ...
              <form onSubmit={handleSubmit}>
                ...
                >
                  <Typography variant="body1">I am signing up as a:</Typography>
                  <Box
                    sx={{
                      display: "inline-flex",
                      alignItems: "center",
                  ...
                  >
                    ...
                    />
                    <span style={{ fontWeight: "normal" }}>Manager</span>
                  </Box>
                  <Box
                    sx={{
                      display: "inline-flex",
